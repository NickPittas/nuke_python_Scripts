...
class RenderProgressPanel(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        """
        Initializes the RenderProgressPanel UI and variables.
        """
        ...
        self.thread_widgets = {}
        self.threads = []
        self.is_rendering = False  # Initialize before calling init_ui()
        self.progress_lock = Lock()  # For thread synchronization
        self.settings = QSettings('YourCompanyName', 'RenderProgressPanel')  # Replace with your company or script name
        self.init_ui()
...
#################################################################################################
        ...
        self.pause_button.setSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
    ...
    #     """
    #     Updates the banner image size when the panel is resized.
    #     """
    #     super(RenderProgressPanel, self).resizeEvent(event)
    #     if hasattr(self, 'banner_label') and hasattr(self, 'pixmap'):
    #         self.banner_label.setPixmap(self.pixmap.scaledToWidth(self.width(), QtCore.Qt.SmoothTransformation))
    ...
    def start_render(self):
        ...
        for idx in range(num_threads):
            thread_id = idx + 1
            if batch_render_enabled:
                frames_to_render = None  # Frames will be assigned dynamically
            else:
                frames_to_render = frames_per_thread[idx]
            ...
            # Connect the batch_started signal
            render_thread.batch_started.connect(self.reset_thread_progress)
            # Create UI components for each thread
            if batch_render_enabled:
                thread_widget = QtWidgets.QGroupBox(f'Thread {thread_id}')
            else:
                thread_widget = QtWidgets.QGroupBox(f'Thread {thread_id}: Frames {frames_to_render[0]} - {frames_to_render[-1]}')
            vbox = QtWidgets.QVBoxLayout()
            progress_bar = QtWidgets.QProgressBar()
            stats_label = QtWidgets.QLabel('Time per frame: N/A\nEstimated time remaining: N/A')
            # Create collapsible log area
            collapsible_log = CollapsibleWidget(title='Show Log')
            log_text_edit = QtWidgets.QTextEdit()
            log_text_edit.setReadOnly(True)
            collapsible_log.add_widget(log_text_edit)
            # Add widgets to thread layout
            vbox.addWidget(progress_bar)
            vbox.addWidget(stats_label)
            vbox.addWidget(collapsible_log)
            ...
            render_thread.log_text_edit = log_text_edit
            ...
            render_thread.thread = thread  # Keep a reference to the thread
    ...
    def reset_thread_progress(self, thread_id, total_frames):
        ...
        if render_thread:
            ...
            render_thread.stats_label.setText('Time per frame: N/A\nEstimated time remaining: N/A')
    ...
    def update_progress(self, current_frame, total_frames, time_per_frame, thread_id):
        """
        Updates the progress bars and statistics.
        """
        with self.progress_lock:
            render_thread = next((rt for rt in self.render_threads if rt.thread_id == thread_id), None)
            if render_thread:
                frames_rendered = min(render_thread.frames_rendered, render_thread.total_frames)
                total_frames_thread = render_thread.total_frames
                progress = int((frames_rendered / total_frames_thread) * 100) if total_frames_thread else 0
                render_thread.progress_bar.setValue(progress)
                estimated_time_remaining = max(0, time_per_frame * (total_frames_thread - frames_rendered))
                ...
                # Store time per frame and frames rendered
                render_thread.time_per_frame = time_per_frame

            # Update overall progress
            self.total_frames_rendered = sum(rt.total_frames_rendered for rt in self.render_threads)
    ...
    def render_complete(self, total_duration, thread_id):
        ...
        if render_thread:
            ...
            render_thread.log_text_edit.append("Render complete.")
            # Clean up the thread
            render_thread.thread.quit()
            render_thread.thread.wait()
    ...
    def render_stopped(self, thread_id):
        ...
        render_thread = next((rt for rt in self.render_threads if rt.thread_id == thread_id), None)
        if render_thread:
            render_thread.stats_label.setText(render_thread.stats_label.text() + '\nRender Stopped')
            render_thread.log_text_edit.append("Render stopped.")
            # Clean up the thread
            render_thread.thread.quit()
            render_thread.thread.wait()
    ...
    def update_log(self, message, thread_id):
        """
        Updates the log text edit with messages from render threads.
        """
        render_thread = next((rt for rt in self.render_threads if rt.thread_id == thread_id), None)
        if render_thread:
            render_thread.log_text_edit.append(message)
